...
let%shared () =
  ...
  Eliom_registration.Any.register ~service:Os_services.action_link_service
    (Os_session.Opt.connected_fun Myapp_handlers.action_link_handler);
  ...
  Eliom_registration.Action.register
    ...
    Myapp_handlers.update_language_handler;
  Myapp_base.App.register ~service:Os_services.main_service
    (Myapp_page.Opt.connected_page Myapp_handlers.main_service_handler);
  Myapp_base.App.register ~service:Myapp_services.about_service
    (Myapp_page.Opt.connected_page Myapp_handlers.about_handler);
  Myapp_base.App.register ~service:Myapp_services.settings_service
    (Myapp_page.Opt.connected_page Myapp_handlers.settings_handler)

let%server () =
  Eliom_registration.Ocaml.register
    ~service:Myapp_services.upload_user_avatar_service
    (Os_session.connected_fun Myapp_handlers.upload_user_avatar_handler)

(* Print more debugging information when <debugmode/> is in config file
   (DEBUG = yes in Makefile.options).
   Example of use:
   let section = Lwt_log.Section.make "Myapp:sectionname"
   ...
    ...
    ignore
      [%client
        ((* Eliom_config.debug_timings := true; *)
         ...
         Lwt_log_core.add_rule "Myapp*" Lwt_log_js.Debug
         (* Lwt_log_core.add_rule "*" Lwt_log_js.Debug *)
         : unit)];
    (* Lwt_log_core.add_rule "*" Lwt_log.Debug *)
    Lwt_log_core.add_rule "Myapp*" Lwt_log.Debug)
...
module Myapp_config = Myapp_config
module Myapp_language = Myapp_language
